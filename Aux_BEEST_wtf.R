...
read_prep = function(n_chains) {

	for(i in 1:n_chains){
		chain = read.csv(file=paste(paste("parameters",i,sep=""),".csv",sep=""),head=TRUE,sep=";")
		my_chain = as.matrix(chain[,order(names(chain))])
		n_subject = ifelse(ncol(my_chain)==7,1,(ncol(my_chain)-14)/7)
...
summary_stats_individual = function(pars, params, subject_idx = NULL){
  ...
  # Write table
	...
	h <- grobHeight(table)
  w <- grobWidth(table)
  title <- textGrob("Summary statistics", y=unit(0.5,"npc") + 0.5*h, 
...
#-----------------------------------------------------------------------------------------------------------------------------------------Computes summary statistics for each parameter per participant (collapsed over chains) and save output to csv.
summary_stats_individual_probit = function(pars, params, subject_idx){
	...
	p_tf_subj = grep(glob2rx("p_tf*"), params, value=TRUE)
	
	for(par in params){
    if(par==p_tf_subj){
    
      summary[p_tf_subj,1] = round(mean(as.vector(pnorm(pars$traces[,p_tf_subj,]))),4)
      summary[p_tf_subj,2] = round(sd(as.vector(pnorm(pars$traces[,p_tf_subj,]))),4)
      summary[p_tf_subj,3:7] = round(quantile(as.vector(pnorm(pars$traces[,p_tf_subj,])),prob = c(0.025,0.25,0.5,0.75,0.975)),4)
    } else {
...
summary_stats_group = function(pars){

	#only group nodes
	group_pars = c("mu_go","mu_go_var","mu_stop","mu_stop_var","sigma_go","sigma_go_var","sigma_stop","sigma_stop_var","tau_go","tau_go_var","tau_stop","tau_stop_var","p_tf","p_tf_var")
	summary = matrix(NA,14,7)
	colnames(summary) = c("Mean","Sd","2.5%","25%","50%","75%","97.5%")
...
summary_stats = function(pars,all_pars=T){
	if (all_pars){
		if (pars$n_subject==1) ###Individual model
			summary_stats_individual(pars, params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop","p_tf"))
		else {                 ####Group model
			summary_stats_group(pars)
			...
			for (n in 1:pars$n_subject){ #Seperate table for each participant
				param_name = c(paste("mu_go_subj.",n,sep=""),paste("mu_stop_subj.",n,sep=""),
                        ...
                        paste("p_tf_subjpt.",n,sep=""))
			
			summary_stats_individual_probit(pars, params = param_name,subject_idx = n)
			}
		} 
	} else summary_stats_group(pars)
}
...
plot_individual_posteriors = function(pars,params,priors){
  ...
  prior_den = list(mu_go = dunif(priors$mu_go[1],priors$mu_go[1],priors$mu_go[2]), 
                    ...
                    p_tf = dunif(priors$p_tf[1],priors$p_tf[1],priors$p_tf[2]))
  ...
  plot(density(pars$traces[,"p_tf",]),xlim = c(priors$p_tf[1],priors$p_tf[2]), main = "Posterior p_tf",xlab="Probability",ylab = "Density")
  lines(c(priors$p_tf[1],priors$p_tf[2]),c(prior_den[names(prior_den)=="p_tf"],prior_den[names(prior_den)=="p_tf"]),lty=2)
 }
...
plot_individual_posteriors_for_hier = function(pars,params,subject_idx){
  ...
  plot(density(pnorm(pars$traces[,p_tf_subj,])),xlim = c(0,1), main = paste("Posterior", p_tf_subj,sep=" "),xlab="Probability",ylab = "Density")  
}
...
plot_individual_posterior_means = function(pars,priors){
  ...
  nam = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop","p_tf")
...
library(msm)
p_lims_prior = seq(priors$p_tf[1],priors$p_tf[2],0.1)
  ...
  prior_den_group = list(mu_go = dunif(priors$mu_go[1],priors$mu_go[1],priors$mu_go[2]), 
                    ...
                    tau_go = dunif(priors$tau_go[1],priors$tau_go[1],priors$tau_go[2]),
                    tau_stop = dunif(priors$tau_stop[1],priors$tau_stop[1],priors$tau_stop[2]),
                    p_tf = dtnorm(p_lims_prior,priors$p_tf[3],priors$p_tf[4],lower=priors$p_tf[1],upper=priors$p_tf[2]),
                    ...
                    p_tf_var = dunif(priors$p_tf_var[1],priors$p_tf_var[1],priors$p_tf_var[2]))
  ...
  plot(density(pars$traces[,"p_tf",]),xlim = c(priors$p_tf[1],priors$p_tf[2]), main = "Posterior p_tf",xlab="p_tf on probit scale",ylab = "Density")
  lines(p_lims_prior,as.numeric(unlist(prior_den_group[names(prior_den_group)=="p_tf"])),lty=2)

  plot(density(pars$traces[,"p_tf_var",]),xlim = c(priors$p_tf_var[1],priors$p_tf_var[2]), main = "Posterior p_tf_var",xlab="p_tf_var on probit scale",ylab = "Density")
  lines(c(priors$p_tf_var[1],priors$p_tf_var[2]),c(prior_den_group[names(prior_den_group)=="p_tf_var"],prior_den_group[names(prior_den_group)=="p_tf_var"]),lty=2)
}
...
plot_posteriors = function(pars,all_pars=T,priors){
	if (all_pars){
		if (pars$n_subject==1) ###Individual model
			plot_individual_posteriors(pars, params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop", "p_tf"), priors) 
...
plot_individual_chains = function(pars,params,priors,subject_idx = NULL){
  ...
  p_tf = c(priors$p_tf[1],priors$p_tf[2])
  ...
  par(mar = c(8, 5, 6, 3) + 0.1)
	...
	plot(1:pars$n_samples,pars$traces[,"p_tf",1],ylim = c(p_tf[1],p_tf[2]), xlim = c(1,pars$n_samples), main = "MCMC chains p_tf",xlab="Iteration",ylab = "p_tf",type="l")
		
	if(pars$n_chains>1){  #if multiple chains, draw lines
			for(j in 2:pars$n_chains){
				lines(1:pars$n_samples,pars$traces[,"p_tf",j],col=j)
	  	}
	}
}
...
plot_individual_chains_probit = function(pars,params,subject_idx = NULL){
  ...
  p_tf_subj = grep(glob2rx("p_tf*"), params, value=TRUE)
	...
	plot(1:pars$n_samples,pnorm(pars$traces[,p_tf_subj,1]),ylim = c(0,1), xlim = c(1,pars$n_samples), main = paste("MCMC chains",p_tf_subj,sep=" "), xlab="Iteration",ylab = p_tf_subj,type="l")
		
	if(pars$n_chains>1){  #if multiple chains, draw lines
			for(j in 2:pars$n_chains){
				lines(1:pars$n_samples,pnorm(pars$traces[,p_tf_subj,j]),col=j)
			}
	}
}
...
plot_group_chains = function(pars,priors){
  ...
  plot(1:pars$n_samples,pars$traces[,"p_tf",1],ylim = c(priors$p_tf[1],priors$p_tf[2]), xlim = c(1,pars$n_samples), main = "MCMC chains p_tf",xlab="Iteration",ylab = "p_tf",type="l")
  ...
  plot(1:pars$n_samples,pars$traces[,"p_tf_var",1],ylim = c(priors$p_tf_var[1],priors$p_tf_var[2]), xlim = c(1,pars$n_samples), main = "MCMC chains p_tf_var",xlab="Iteration",ylab = "p_tf_var",type="l")
		
	if(pars$n_chains>1){ #if multiple chains, draw lines
			for(j in 2:pars$n_chains){
				lines(1:pars$n_samples,pars$traces[,"p_tf_var",j],col=j)
			}
	}	
}
...
plot_chains = function(pars,all_pars=T,priors){
	if (all_pars){
		if (pars$n_subject==1) #Individual model
			plot_individual_chains(pars, params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop","p_tf"),priors)
		else {                 #Group model
...
sample_joint_posterior = function(pars,n_post_samples,subject_idx = NULL){
	...
	if(pars$n_subject==1){
		params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop","p_tf")
	} else {
		params = c(paste("mu_go_subj.",subject_idx,sep=""),paste("mu_stop_subj.",subject_idx,sep=""),
				...
				paste("tau_go_subj.",subject_idx,sep=""),paste("tau_stop_subj.",subject_idx,sep=""),paste("p_tf_subjpt.",subject_idx,sep=""))
	}
...
generate_posterior_predictions_srrt = function(delays_all,n_delays_all,n_observed_all,n_observed_srrt_all,par_vectors,n_post_samples,params,n_subj,subject_idx=NULL){
	...
	if(n_subj==1){
		prob = par_vectors[,"p_tf"]
	} else {
    prob = pnorm(par_vectors[,paste("p_tf_subjpt.",subject_idx,sep="")])
  }
  #print(prob)
  #print(n_delays_all)
...
post_pred_output_inhibition_function = function(params,pars,n_subj,n_delays_all,delays_all,RR_pred,RR_obs,n_observed_all,subject_idx=NULL){
  ...
  pvalue_one_if = rep(NA,n_delays_all)
    ...
    prob = pnorm(pars$traces[,grep(glob2rx("p_tf*"), params, value=TRUE),])
  }
