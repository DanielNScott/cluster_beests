...
plot_group_posteriors = function(pars,priors){
  ...
  for(par in prior_names_var){
    ...
    flog.info('Plotting posterior for %s', par)
    title <- paste("Posterior",par,sep=" ")
    ...
    if (par == 'pf_stop_var' | par == 'pf_stop') {browser()}
...
plot_group_chains = function(pars,priors){
  ...
  group_pars = c("mu_go","mu_go_sd","mu_stop","mu_stop_sd",
                "sigma_go","sigma_go_sd", "sigma_stop","sigma_stop_sd",
                 "tau_go","tau_go_sd","tau_stop","tau_stop_sd")
  ...
  plot(1:pars$n_samples,pars$traces[,"pf_stop_sd",1],ylim = c(priors$pf_stop_sd[1],priors$pf_stop_sd[2]), xlim = c(1,pars$n_samples), main = "MCMC chains pf_stop_sd",xlab="Iteration",ylab = "pf_stop_sd",type="l")

  if(pars$n_chains>1){ #if multiple chains, draw lines
      for(j in 2:pars$n_chains){
        lines(1:pars$n_samples,pars$traces[,"pf_stop_sd",j],col=j)
      }
  }
}
