...
class StopSignal(Hierarchical):
    ...
    def create_knodes(self):
        ...
        knodes.update(self.create_family_tau('tau_stop'))
    ...
    def create_family_tau(self, name, value=50, mean_gr=40, var_gr=50, lower=1, upper=500, var_lower=1e-10,var_upper=200, var_value=50):
        knodes = OrderedDict()
        ...
        if self.is_group_model and name not in self.group_only_nodes:
            ...
            var = Knode(pm.Uniform, '%s_var' % name, lower=var_lower,upper=var_upper, value=var_value)
            tau = Knode(pm.Deterministic, '%s_tau' % name,doc='%s_tau' % name, eval=lambda x: x**-2, x=var,plot=False, trace=False, hidden=True)
            subj = Knode(pm.TruncatedNormal, '%s_subj' % name, mu=g,tau=tau, a=lower, b=upper, value=value, depends=('subj_idx',), subj=True,plot=self.plot_subjs)         
            ...
            subj = Knode(pm.Uniform, name, lower=lower,upper=upper, value=value,depends=self.depends[name])
