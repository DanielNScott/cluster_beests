...
summary_stats = function(pars,all_pars=T){
	if (all_pars){
		if (pars$n_subject==1) ###Individual model
			summary_stats_individual(pars, params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop"))
		else {                 ####Group model
...
plot_individual_posterior_means = function(pars,priors){
  nsub = pars$n_subject
  post_means_ind = array(NA,dim=c(nsub, 6))
  nam = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop")
...
plot_group_posteriors = function(pars,priors){
  ...
  prior_den_group = list(mu_go = dunif(priors$mu_go[1],priors$mu_go[1],priors$mu_go[2]), 
	...
	layout(matrix(1:12,,2,byrow=T))
	par(cex.main=1.2)
	
	group_pars = c("mu_go","mu_go_var","mu_stop","mu_stop_var","sigma_go","sigma_go_var","sigma_stop", "sigma_stop_var","tau_go","tau_go_var",
                "tau_stop","tau_stop_var")
...
plot_posteriors = function(pars,all_pars=T,priors){
	if (all_pars){
		if (pars$n_subject==1) ###Individual model
			plot_individual_posteriors(pars, params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop"), priors) 
...
plot_chains = function(pars,all_pars=T,priors){
	if (all_pars){
		if (pars$n_subject==1) #Individual model
			plot_individual_chains(pars, params = c("mu_go","mu_stop","sigma_go","sigma_stop","tau_go","tau_stop"),priors)
		else {                 #Group model
...
generate_posterior_predictions_srrt = function(delays_all,n_delays_all,n_observed_all,n_observed_srrt_all,par_vectors,n_post_samples,params,n_subj,subject_idx=NULL){
    ...
    n_notf = n_observed_all[d]
  
    for(j in 1:n_post_samples){
      if(n_notf>0){ 
        ###triggered trials
        go_rts = rexGAUS(n_notf,mu = par_vectors[j,params[1]],sigma = par_vectors[j,params[3]],nu = par_vectors[j,params[5]])
        stop_rt = delays_all[d] + rexGAUS(n_notf,mu=par_vectors[j,params[2]],sigma = par_vectors[j,params[4]],nu = par_vectors[j,params[6]])
        srrt = go_rts[go_rts<=stop_rt]
       } else srrt = NULL
       
      post_pred_srrt[[d]][[j]] = list(srrt)
      median_post_pred_srrt[d,j] = median(srrt)	
      RR_pred[d,j] = (length(srrt))/n_observed_all[d]
    }
   }
